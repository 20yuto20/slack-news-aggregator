...
class FirestoreClient:
    ...
    def _initialize_firebase(self):
        ...
        """
        # テスト環境（ENVIRONMENT=test）の場合は初期化をスキップ
        ...
        if not firebase_admin._apps:
            ...
            absolute_cred_path = project_root / cred_path
            
            if not absolute_cred_path.exists():
                raise FileNotFoundError(f"Credentials file not found at: {absolute_cred_path}")
    ...
    # --------------------------------------------------------------------
    # 新規追加メソッド: 過去記事 (全件) を取得するため
    # --------------------------------------------------------------------
    def get_all_articles_by_company_id(self, company_id: str) -> List[Dict[str, Any]]:
        """
        指定したcompany_idの過去記事をすべて取得して返す
        (statusフィールドは特に絞り込まずに全件)
        """
        articles_ref = self.db.collection(self.config['collections']['articles']['name'])
        docs = articles_ref.where('company_id', '==', company_id).stream()
        results = []
        for doc in docs:
            data = doc.to_dict()
            data['id'] = doc.id
